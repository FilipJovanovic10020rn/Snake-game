...
void rafgl_raster_draw_spritesheet_smrt_zmije(rafgl_raster_t *raster, rafgl_spritesheet_t *spritesheet, int sheet_x, int sheet_y, int x, int y)
{
    int fl, fr, fu, fd;
    int flc, frc, fuc, fdc;
    int xi, yi;
    ...
    for(yi = fuc; yi < fdc; yi++)
    {
        for(xi = flc; xi < frc; xi++)
        {
            ...
            {
                pixel_at_pm(raster, xi, yi) = sampled;
                if(pixel_at_pm(raster,xi,yi).rgba == rafgl_RGB(0,59,255)){
                    pixel_at_pm(raster, xi, yi).rgba = rafgl_RGB(219,0,202);
                }
                else if(pixel_at_pm(raster,xi,yi).rgba == rafgl_RGB(6,27,99)){
                    pixel_at_pm(raster, xi, yi).rgba = rafgl_RGB(219,129,202);
                }
            }
        }
    }
